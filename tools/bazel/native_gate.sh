#!/usr/bin/env bash
set -euo pipefail

script_source="${BASH_SOURCE[0]}"
if command -v realpath >/dev/null 2>&1; then
  script_path="$(realpath "$script_source")"
else
  script_path="$(python3 -c 'import os, sys; print(os.path.realpath(sys.argv[1]))' "$script_source")"
fi

repo_root="$(git -C "$(dirname "$script_path")" rev-parse --show-toplevel 2>/dev/null || true)"
if [[ -z "$repo_root" ]]; then
  repo_root="$(cd "$(dirname "$script_path")/../.." && pwd)"
fi

cd "$repo_root"
case "$(uname -s)" in
  Darwin)
    exec Scripts/check_counter_demo.sh --skip-clean
    ;;
  Linux)
    exec Scripts/check_counter_demo.sh --linux-only --skip-clean
    ;;
  *)
    printf 'unsupported counter demo native gate platform: %s\n' "$(uname -s)" >&2
    exit 1
    ;;
esac
